...
class CycleGan(keras.Model):
    ...
    def train_step(self, batch_data):
        ...
        def gp(real_img, fake_image, net):
            interpolates = real_img + fake_image
        ...
        for _ in range(5):
            with tf.GradientTape(persistent=True) as tape:
                ...
                monet_disc_loss += gp(real_monet, fake_photo, self.m_disc)
                photo_disc_loss += gp(real_photo, fake_monet, self.p_disc)
...
with strategy.scope():
    monet_generator_optimizer = tf.keras.optimizers.Adam(2e-4, beta_1=0.8)
    photo_generator_optimizer = tf.keras.optimizers.Adam(2e-4, beta_1=0.8)

    monet_discriminator_optimizer = tf.keras.optimizers.Adam(5e-6, beta_1=0.8)
    photo_discriminator_optimizer = tf.keras.optimizers.Adam(5e-6, beta_1=0.8)
